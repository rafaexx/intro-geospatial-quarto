...
pacman::p_load("rmarkdown", "reticulate", "tidyverse", "readxl", "sf", "stars", "rnaturalearth", "mapview", "skimr", "glue", "raster", "fasterize", "purrr")
```

Define a shortcut to a folder that you may very often need, so later you can use shorter paths to your files

```{r}
# be aware that R uses / (forward slashes)
my_gis_lib <- "C:/Users/Rafael.Camargo/wwfgermany/FB-OuR-Daten - WRF/ws_gis/1_library/" 
# that's an example
paste0(my_gis_lib, "Wetlands/TROP-SUBTROP_WetlandV3b_2016_CIFOR.tif") 
```

This time you download all the play data for this exercise [here](https://drive.google.com/file/d/1-yfH-WoiBJjhSDcJnvIzbT8I6QsYxnn6/view?usp=sharing)
...
```{r}
ramsar <- readr::read_csv("data/ramsar_sites.csv") %>%
...
```{r}
cities <- readxl::read_excel("data/cities_over300k.xls", sheet = "Data", skip = 16) %>%
  sf::st_as_sf(coords = c("Longitude","Latitude"), crs = 4326, remove = FALSE)
```
...
```{r}
world_wrf <- sf::st_read("data/shp/WRF_GDP_country.shp")
```

From geodatabase

```{r}
portugal_rbd <- sf::st_read("data/portugal.gdb", layer = "eu_river_basin_district")
```

From geojson

```{r}
portugal_adm1 <- sf::st_read("data/portugal_adm1.geojson")
```

From raster

```{r}
ncp <- stars::read_stars("data/raster/avocado_Production.tif")
```
...
The [sf](https://r-spatial.github.io/sf/) package as well as the [dplyr, tidyrr, stringr, forcats, and glue](https://www.tidyverse.org/packages/) are your best friend here
...
get_top_producing_regions <- function(input, output){
  ...
  crop <- stars::read_stars(paste0("data/raster/", input))
  crop[crop == 0] <- NA
  ...
  temp <- sf::st_as_sf(crop) %>%
    sf::st_combine() %>%
    sf::st_write(dsn = "data/shp",  output, driver = "ESRI Shapefile", delete_layer = TRUE)
} 
```
...
```{r}
st_read("data/shp/avocado_top_prod.shp") %>%
  mapview::mapview()
```
...
```{r}
check_before_download(destfile = "data/spei48.nc",
                      url = "https://soton.eead.csic.es/spei/10/nc/spei48.nc")
```

#### Handle the data

Load as a multi-layer raster object

```{r}
spei <- raster::brick("data/spei48.nc")
```

Get the last 10 years of monthly observations, where Z is the variable time

```{r}
spei_sliced <- spei[[which(
  raster::getZ(spei) >= seq(Sys.Date(), length = 2, by = "-122 months")[2] & raster::getZ(spei) <= Sys.Date()
)]]

length(spei_sliced[4])
```
...
drought_probability <- raster::calc(spei_sliced, fun = below_threshold) %>%
  sum() / length(spei_sliced[4])
```
